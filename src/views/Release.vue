...
<template>
  <div v-if="release">
    ...
    <div class="split-layout">
      <!-- COLONNE GAUCHE (Maintenant correctement animée par le CSS) -->
      <div class="split-left">
        ...
        <div class="release-actions" v-if="release.links?.length">
          <a 
            v-for="(link, index) in release.links" 
            :key="index" 
            :href="link.url" 
            target="_blank" 
            class="gato-btn full-width"
            :class="{ 'green-btn': index === 0 }"
          >
      ...
      <div class="split-info">
        ...
        <h2 class="split-subtitle">
          <router-link v-if="artistInfo.id" :to="'/artist/' + artistInfo.id" class="artist-link">
            {{ artistInfo.name }}
          </router-link>
